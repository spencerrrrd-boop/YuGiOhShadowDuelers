    ...
    public partial class FormBattle : Form
    {
        ...
        private void SetupButtons()
        {
            ...
            btnStart.Click += (s, e) =>
            {
                ...
                _turnPhase          = -1;
                _turnTimer.Interval = 800;  // short pause then first turn
                _turnTimer.Start();
            };
        ...
        // ── Turn logic — phase-based so each sprite state is visible ─────────
        // Each turn has 4 phases (500ms each = 2000ms total per turn):
        //   Phase 0: player attacks   → player=attack,  kaiba=idle
        //   Phase 1: kaiba reacts     → player=idle,    kaiba=hit/stunned/idle
        //   Phase 2: kaiba attacks    → kaiba=attack,   player=idle
        //   Phase 3: player reacts    → kaiba=idle,     player=hit/stunned/idle
        // Then next turn starts.

        private TurnLog?  _pendingPlayerLog = null;
        private TurnLog?  _pendingKaibaLog  = null;
        private int       _turnPhase        = -1;   // -1 = waiting for next turn
        ...
        private void TurnTimer_Tick(object? sender, EventArgs e)
        {
            ...
            // Phase 0: calculate new turn, show player attacking
            if (_turnPhase == 0)
            {
                if (_engine.IsGameOver) { EndGame(); return; }

                (_pendingPlayerLog, _pendingKaibaLog) = _engine.ExecuteTurn();

                // Log messages
                if (_pendingPlayerLog != null)
                {
                    string pMsg = BuildMessage(_pendingPlayerLog, isPlayerAttacking: true);
                    _log.Insert(0, $"[T{_pendingPlayerLog.TurnNumber}] {pMsg}");
                    _lastMessage = pMsg;
                }
                if (_pendingKaibaLog != null)
                {
                    string kMsg = BuildMessage(_pendingKaibaLog, isPlayerAttacking: false);
                    _log.Insert(0, $"      ↩ {kMsg}");
                }
                if (_log.Count > 9) _log.RemoveRange(9, _log.Count - 9);

                // Sprites: player attacks → kaiba idle
                if (_pendingPlayerLog != null)
                {
                    bool playerSkipped = _pendingPlayerLog.RawDamage == 0 && !_pendingPlayerLog.WasDodged;
                    _playerState = playerSkipped ? "idle" : "attack";
                    _kaibaState  = "idle";
                }

                _turnTimer.Interval = 600;  // show attack pose for 600ms
            }
            // Phase 1: show kaiba reacting to player's hit
            else if (_turnPhase == 1)
            {
                _playerState = "idle";
                if (_pendingPlayerLog != null)
                {
                    if      (_pendingPlayerLog.WasDodged)    _kaibaState = "idle";
                    else if (_pendingPlayerLog.WasMaxDamage) _kaibaState = "stunned";
                    else if (_pendingPlayerLog.NetDamage > 0) _kaibaState = "hit";
                    else                                      _kaibaState = "idle";
                }
                _turnTimer.Interval = 500;
            }
            // Phase 2: kaiba attacks
            else if (_turnPhase == 2)
            {
                _kaibaState = "idle";
                if (_pendingKaibaLog != null)
                {
                    bool kaibaSkipped = _pendingKaibaLog.RawDamage == 0 && !_pendingKaibaLog.WasDodged;
                    _kaibaState  = kaibaSkipped ? "idle" : "attack";
                    _playerState = "idle";
                }
                else
                {
                    // No kaiba log = game already over after player hit
                    _turnPhase = -1;
                    _turnTimer.Interval = 300;
                    if (_engine.IsGameOver) { EndGame(); return; }
                }
                _turnTimer.Interval = 600;
            }
            // Phase 3: player reacts to kaiba's hit
            else if (_turnPhase == 3)
            {
                _kaibaState = "idle";
                if (_pendingKaibaLog != null)
                {
                    if      (_pendingKaibaLog.WasDodged)     _playerState = "idle";
                    else if (_pendingKaibaLog.WasMaxDamage)  _playerState = "stunned";
                    else if (_pendingKaibaLog.NetDamage > 0)  _playerState = "hit";
                    else                                      _playerState = "idle";
                }
                _turnTimer.Interval = 500;
            }
            // Phase 4: reset to idle before next turn
            else
            {
                _playerState   = "idle";
                _kaibaState    = "idle";
                _turnPhase     = -1;
                _turnTimer.Interval = 400;  // brief idle before next turn starts
